    ...
    <div className="space-y-12">
      <section>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {active.map(p=>(
            <button key={p.slug} onClick={() => onProjectClick(p.slug)} className="text-left card ring-1 ring-gray-200 rounded-lg p-5 hover:shadow">
              <h3 className="font-semibold">{p.title}</h3>
              <p className="text-sm text-gray-600 mt-1">{p.stats.total_results} results • {Object.entries(p.stats.by_severity).map(([k,v])=>`${k}:${v}`).join("  ")}</p>
            </button>
          ))}
        </div>
      </section>
      <section>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {past.map(p=>(
            <button key={p.slug} onClick={() => onProjectClick(p.slug)} className="text-left card ring-1 ring-gray-200 rounded-lg p-5 hover:shadow">
              <h3 className="font-semibold">{p.title}</h3>
              <p className="text-sm text-gray-600 mt-1">{p.stats.total_results} results</p>
            </button>
          ))}
        </div>
      </section>
